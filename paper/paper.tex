...
and whether it is \emph{contested}. The oracle that produced a claim is a \emph{generic type} --- a prover, a test
runner, a static analyzer, a database queried for a fact, an LLM judge, a human sign-off --- and every result
carries the honest \emph{strength} of its guarantee (proof $>$ sat $>$ tests $>$ static analysis $>$ attested),
so a goal reports the weakest link it rests on; a requirement may point at a \emph{reference} model outside the
repository --- a regulation, a venue specification --- and evidence against it goes stale when the reference
moves.
...
Six failures, all but invisible to testing and review as practiced today, motivate the construction. Each is
...
\paragraph{6. The rule that moved.} A payment service was checked, thoroughly, against the provider's
specification of its PaymentIntent lifecycle; a reporting pipeline was checked against an article of the
regulation it implements. Then the provider re-issued the specification, or the regulator amended the article.
No file in the repository changed, so no test, no diff and no freshness check keyed on the code notices;
every ``conforms'' still reads green against a text that no longer says what it said. A requirement that points
at a model \emph{outside} the repository must record what it was judged against (\S\ref{sec:goals-reference}),
and the reference must be a dependency like any other, so that a revision to it stales the evidence
(\S\ref{sec:freshness-reference}).
...
\item \textbf{Graded evidence from any oracle} (\S\ref{sec:oracles}): the oracle as a generic type with an
honest strength ladder and a subject fingerprint, so a proof, a test run, a database lookup and a sign-off are
governed evidence on one footing; and \textbf{requirements against a reference} (\S\ref{sec:goals-reference}),
the binding of a repository to a model outside it, with reference freshness (\S\ref{sec:freshness-reference}).
...
\item The system (\S\ref{sec:impl}) --- the CLI, the policy checker, the one overview every screen reads
and the continuous-integration action --- its trust, audit and review surfaces (\S\ref{sec:trust}), its
interchange and sync (\S\ref{sec:interchange}), and the \emph{multi-reasoner} generalization
(\S\ref{sec:multireasoner}).
\end{itemize}
...
\subsection{The record as it is read}\label{sec:impl-overview}

Everything above is computed \emph{over} the trace; a person, and an agent between turns, reads a
\emph{summary} of it. Every screen that summary reaches --- a terminal, an agent's own prompt, a desktop,
the pull-request comment of \S\ref{sec:impl-action} --- speaks five nouns and their states
(Table~\ref{tab:five-words}). The formal terms of this paper stay in the record; the five words are how the
record is read. What makes this more than a style guide is \emph{where the states are computed}: once, by
\code{ponens}, and never by a renderer. \code{ponens trace overview} returns one JSON with every requirement
and its state, the evidence and grade behind it, every open gap with its state, the gate with the rules that
block it, what to do next, the evidence per symbol, and the record's health; \code{ponens trace
requirements} is the requirements part alone, the join of a repository's requirements file with the trace's
goals, reference artifacts, conformance results and freshness verdicts under seven ordered row rules;
\code{ponens trace blame} is \code{git blame} for what was established --- per code symbol, the strongest
fresh result and the gaps anchored to it; \code{ponens trace next} is the record's own to-do list, ordered
fix, meet, refresh, gap, optional; and \code{ponens trace integrity} names the established evidence a
proposed new version of the record no longer holds, so a save hook can refuse to weaken a record quietly.
A renderer paints these; it derives nothing. Two consequences follow. Every surface agrees, because there
is one place to disagree with. And the vocabulary is enforced: the reference renderers lint their own output
for the retired terms, so ``residual'' or ``conformance'' cannot leak back onto a screen, and the one rule
that must never break --- ``tested'' does not read as ``proved'' --- holds by construction.
...
Word & Absorbs & States \\
\midrule
requirement & goal acceptance item, binding (a reference artifact and its conformance items), invariant & met · open · failed · out of date \\
evidence & verification, conformance, state-space analysis, tests, attestation & proved · witnessed · tested · checked · attested; fresh · out of date \\
gap & residual (assumption, limitation, defeater, open question, unverified), stale or detached evidence & missing · assumed · failed · out of date \\
record & the trace & saved · frozen · combined \\
gate & the policy evaluations & pass · blocked by \ldots \\
\bottomrule
\end{tabularx}
\caption{How the record is read: the five words every rendered surface speaks, what each absorbs from the
formal vocabulary, and its states. Plus \emph{model} for what a requirement points at. The states are
computed once by \code{ponens trace overview}; no renderer derives one.}
\label{tab:five-words}
\end{table}

\subsection{Continuous integration: the Action}\label{sec:impl-action}

The record reaches the review it was made for through the pull request. A GitHub Action
(\code{imandra-ai/ponens/action}) finds the single trace an agent published under \code{.ponens/} --- or
reconstructs one from a session transcript with \code{ponens emit} when none was published --- validates it,
runs the policy gate over the policies the trace carries plus any the workflow names, resolves the goals
(met or open, the weakest grade each rests on, what is at risk, the open gaps), runs the requirements check
of \S\ref{sec:impl-overview} when the repository carries a requirements file, and reports: in the job
summary, as a file, and on the pull request as \emph{one} comment updated in place on every push. A failed
error-severity policy or a blocked requirements gate fails the job under the workflow's \code{fail-on}, so
the check can be made required and merges blocked on it. Nothing leaves the runner and the repository is
never modified; the repository dogfoods the action on itself. This is the seam at which the two review
surfaces meet: the diff on the platform, the warrant beside it.

\input{sec-trust}

\input{sec-interchange}
...
not attempt. \emph{Software supply-chain attestation.} in-toto~\cite{intoto} and SLSA~\cite{slsa} sign
statements about how an artifact was built --- the steps, their materials and products --- and verify a chain
of them against a layout; they are the nearest prior art to the signature and audit surfaces of
\S\ref{sec:trust}. An attestation records that a step ran; a trace records what the step \emph{established},
how strongly, on what it depends and what it left open, and the same signature discipline
(\S\ref{sec:interchange-sign}) applies over that richer statement.
...
\bibitem{intoto} Torres-Arias, S., Afzali, H., Kuppusamy, T.\ K., Curtmola, R., Cappos, J.\ \emph{in-toto: Providing farm-to-table guarantees for bits and bytes.} USENIX Security, 2019.
\bibitem{slsa} OpenSSF.\ \emph{Supply-chain Levels for Software Artifacts (SLSA), version 1.0.} \url{https://slsa.dev}, 2023.
