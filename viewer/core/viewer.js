...
function loadTrace(data) {
  ...
  document.getElementById('testCount').textContent = collectTests().reduce((s, g) => s + g.tests.length, 0);
...
function actionCardHTML(a, vgByAction) {
  ...
  if (a.decomposition) {
    const nRegions = a.decomposition.regions.length;
  ...
  if (a.generated_tests) {
    const nTests = a.generated_tests.tests.length;
...
function selectAction(actionId) {
  ...
  if (d.generated_tests) {
    ...
    html += `<div class="detail-section">
      <p class="label">Generated Tests: <span class="clr-purple" style="font-family:'SF Mono','Fira Code',monospace;text-transform:none;">${esc(gt.function)}</span> (${esc(gt.language)})</p>`;
  ...
  for (const a of actions) {
    ...
    if (a.generated_tests) {
      groups.tests.items.push({
        action: a,
        label: 'Function under test: ' + a.generated_tests.function,
        badge: a.generated_tests.language,
        meta: a.generated_tests.tests.length + ' tests'
      });
    }
