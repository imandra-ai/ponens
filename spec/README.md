...
|---|---|---|---|
| [`TRACE_SPEC_v1_11.md`](TRACE_SPEC_v1_11.md) | **1.11** | Draft | **The core.** The typed reasoning-trace format — actions, **meta-actions** (§8.4), artifacts & lineage (§7), the **residual surface** (§13), **goals & acceptance** (§18), reproducibility (§12). Everything else is a companion over this. |
...
| [`ORACLE_SPEC_v0_1.md`](ORACLE_SPEC_v0_1.md) | 0.1 | Draft | **Who produced the evidence.** Generalizes *reasoner* to **oracle** — any invocable evidence producer (formal reasoner, test runner, static analyzer, LLM-judge, human attestor) — with two orthogonal classifiers that travel with the claim: `oracle_type` (the mechanism) and `evidence_strength` (the guarantee: proof > sat > tests > static_analysis > attested). A *reasoner* is the formal, proof-producing subtype. |
