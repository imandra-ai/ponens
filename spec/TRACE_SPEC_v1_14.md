...
> what evidence, and at which action - and hanging off the residual in the lineage DAG. The residual
> itself is never edited; a trace's **effective** residual status is *derived* by applying the
> resolutions in action order, so a consumer that reads `status` is unaffected while the declaration and
> its closure both stay in the record. Superseding a resolution is another append, so a waiver that was
> later reversed remains readable as both. A justification is itself a claim, so an open `Defeater`
> (§13.2) aimed at a resolution takes it **out of force** and reopens the gap - invalidating the grounds
> for a closure without removing them from the record. Editing `status` in place stays valid for back-compat but is
> no longer how the CLI closes a gap (`ponens trace residual resolve`). §18.3a applies the same rule to
> the **definition of done**, which is the sharper case - a withdrawn acceptance criterion leaves
> nothing behind at all - with a **`GoalAmendment`** artifact carrying the criterion, the reason and the
> author; and `integrity` now indexes every criterion rather than only the met ones. All additive: a
> trace may carry no resolutions and no amendments, so existing 1.4-1.13 traces remain valid and
> unchanged.
>
...
- `ResidualWaived` records an explicit, auditable decision to accept a gap permanently. Since 1.14
  that decision is itself a record — a `ResidualResolution` artifact (§13.3a) — rather than an edit to
  the residual's `status`.
...
## 13.3a Resolving a residual (additive, 1.14)

A gap is closed by **appending the decision**, not by editing the gap.

Editing a residual's `status` in place is indistinguishable from never having declared it. The record
loses who decided, on what grounds, against what evidence, and at which point in the work; a reader
cannot tell a reasoned waiver from a quiet deletion; and the declaration that made the trace honest is
gone. A trace is append-only everywhere else, and its negative space is no different.

```ocaml
type residual_resolution_payload =
  { residual_id : string              (* the gap being closed *)
  ; status : residual_status          (* Acknowledged | Addressed | Waived - never Open *)
  ; justification : string            (* why: the evidence that closed it, or the grounds for accepting it *)
  ; by : string option                (* who decided *)
  ; at : string option                (* when, as an ISO-8601 instant *)
  ; evidence_artifact_ids : string list  (* what closed it - expected when status = Addressed *)
  }
```

It is recorded in `artifacts` (§7) as:

- `artifact_type = "ResidualResolution"`;
- `derived_from` — the residual it closes, then any cited evidence. This places the closure in the
  lineage DAG hanging off exactly the gap it resolves;
- `producer_action_id` — the action carrying the decision (a `Decision` gateway whose `rationale` is the
  justification), which is what locates the closure *in the work*;
- `summary` — the justification;
- `payload : residual_resolution_payload`.

**Effective status is derived.** A trace's residual surface (§13.1) applies resolutions in **action
order** — by `producer_action_id`, ties in append order — and the last one naming a residual wins. The
residual's own `status` is the base, so a producer that wrote `Waived` directly (valid pre-1.14, and
still valid) is read the same way. A consumer that only reads `status` — every §13.5 policy, a grader, a
review dashboard — therefore needs no change; one that wants the audit trail reads `resolution` (the one
in force) and `resolutions` (the history), and `declared_status` for what the producer originally wrote.

**Reversal is an append.** Superseding a resolution means appending another, so a waiver later withdrawn
stays in the record as a waiver *and* a withdrawal. `Open` is not a resolution status: a residual is born
open, and re-opening one is a decision back to `Acknowledged`, not a pretence that it was never closed.

**A justification can be invalidated without being removed.** A justification is a *claim*, and the way
this model attacks a claim is a `Defeater` (§13.2) — not a deletion. A `ResidualResolution` is an
artifact with an id, so a defeater may target it like any other claim, and while that defeater is
**open** the resolution is **not in force**: the gap it claimed to close reads open again, and the
surface carries `resolution_contested_by`. Both the waiver and the reason it does not hold stay in the
record, so a reader sees the argument rather than a silent edit. Withdrawing the objection is the same
move in reverse — resolve the defeater, and the closure stands again with the whole exchange readable.

A contested closure is *not* the same as a superseded one: superseding says *a different decision was
taken later*, contesting says *this decision was taken on grounds that do not hold*. Both are appends
and both remain readable; only the second is an attack on the reasoning.

Defeating the resolution of a defeater is legal, so the derivation is a bounded iteration rather than a
single pass. Two rules make it total: the bound, and the direction it fails in — a tangle that does not
settle is read **against** closure, because an unresolvable argument about whether a gap is closed is
not a closed gap. A status written directly onto a residual (valid pre-1.14) carries no recorded
justification and so has nothing to contest; superseding it with a resolution is the move there.

**Relationship to successor traces (§13.3).** The two mechanisms answer different questions and compose.
A successor trace closes a gap because *the world changed* — the code was fixed, the proof was
re-established — and the gap no longer appears. A resolution closes a gap because *a judgement was made*
about the gap as it stands, within the trace that declared it. A gap addressed by new evidence in the
same session is a resolution citing that evidence; a gap that outlives the session is addressed by the
successor.
...
## 18.3a Amending the definition of done (additive, 1.14)

The rule of §13.3a applies with more force here. A waived gap still *reads* as a gap; a **withdrawn
acceptance criterion leaves nothing behind at all** — the goal simply reads met, and no reader can tell
that more was once asked. It is the one edit that turns an unmet goal into a met one, and it is
invisible: deleting the single unmet criterion from the shipped Stripe demo moved it from 88% to 100%,
and the `integrity` check reported nothing lost, because it indexed only criteria that were already
`AcceptDone`.

So a change to a goal **appends** a `GoalAmendment` artifact:

```ocaml
type goal_change =
  | ItemWithdrawn      (* an acceptance criterion is no longer part of "done" *)
  | GoalWithdrawn      (* the whole goal is no longer being pursued *)
  | GoalReplaced       (* the definition of done was rewritten *)
  | CriteriaReviewed   (* a non-doer's verdict on whether the criteria are the RIGHT ones *)

type goal_amendment_payload =
  { goal_id : string
  ; change : goal_change
  ; item_id : string option        (* for ItemWithdrawn *)
  ; was : json option              (* the criterion, goal or review AS IT STOOD *)
  ; reason : string                (* why the bar moved — required *)
  ; by : string option
  ; at : string option
  }
```

`was` is the point: **withdrawn from the definition of done is not the same as gone from the record.**
The goal's own `acceptance` list remains the current, unambiguous answer to *what is being asked now* —
consumers need exactly one — while the amendment keeps what was asked before, who changed it, when, and
on what grounds. A reader who sees 100% also sees that the bar moved, and to what.

`CriteriaReviewed` exists for the same reason: a second review used to overwrite the first, so a
`changes-requested` verdict could be replaced by an `approved` one with nothing to show for it. The
current review stays where consumers read it (`criteria_review`); every review is also appended, so the
sequence survives.

**`integrity` (RECORD_OVERVIEW) indexes every criterion**, not only the met ones, so a withdrawal is
reported as a loss across two versions of a record regardless of what state the criterion was in.
