...
const DEMO_TRACES = [
  { name: 'Stripe: 3DS + high-risk approval (proved)', file: 'demo-traces/stripe_v1_1.json' },
...
function loadTrace(data) {
  ...
  // trace with no goals.)
  const showEl = (id, cond) => { const el = document.getElementById(id); if (el) el.style.display = cond ? '' : 'none'; };
  const setCount = (id, n) => { const el = document.getElementById(id); if (el) el.textContent = n; };

  showEl('policiesBtn', !!data.policies?.length);
  showEl('policyViewBtn', !!data.policies?.length);
  setCount('policiesCount', data.policies?.length || 0);

  showEl('residualsBtn', !!data.residuals?.length);
  setCount('residualsCount', data.residuals?.length || 0);

  showEl('dagViewBtn', !!data.artifacts?.length);

  showEl('refModelsBtn', !!data.reference_models?.length);
  setCount('refModelsCount', data.reference_models?.length || 0);
...
// tab has been hidden (the new trace lacks that data), switch back to Flow so no stale pane shows.
function refreshActiveView() {
  const activeBtn = document.querySelector('.view-switcher button.active');
  let view = (((activeBtn && activeBtn.getAttribute('onclick')) || '').match(/switchView\('(\w+)'\)/) || [])[1] || 'flow';
  if (view !== 'flow') {
    const btn = document.querySelector(`.view-switcher button[onclick="switchView('${view}')"]`);
    if (!btn || btn.style.display === 'none') view = 'flow';
  }
  switchView(view); // toggles active tab/pane and re-renders (Flow is already rendered above)
}
...
// Open the evidence behind a goal's acceptance item: an artifact (→ lineage node + detail), a policy
// (→ Policies view), or a residual (→ residual surface). Typed by the trace's own ids, not by prefix.
function openGoalEvidence(id) {
  if (!id) return;
  const sid = String(id);
  if ((traceData?.artifacts || []).some(a => a.artifact_id === sid)) { openArtifactRef(sid); return; }
  if ((traceData?.policies || []).some(p => p.policy_id === sid)
      || (traceData?.policy_evaluations || []).some(p => p.policy_id === sid)) {
    try { switchView('policy'); } catch (e) { /* view may be pinned */ }
    return;
  }
  if ((traceData?.residuals || []).some(r => r.residual_id === sid)) { openModal('residuals', sid); return; }
...
function _artifactJump(id, text) {
  ...
  return `<a href="#" onclick="event.preventDefault();event.stopPropagation();closeModal();switchView('dag');`
    + `selectDAGNode('${esc(id)}');" style="color:var(--accent);text-decoration:none;">${t}</a>`;
}
...
function selectDAGNode(artifactId) {
  ...
  if (residual) {
    ...
    h += `<div class="dd-field">ID: <span>${esc(residual.residual_id)}</span></div>`;
    ...
    // This branch returns before the generic artifact panel below, which is where `Producer` and
    // `Derived From` are rendered - so a residual selected here showed severity, status and a
    // statement and nothing about where it came from, while every other artifact showed both.
    const aid = residual.introduced_by_action_id;
    h += `<div class="dd-label">Surfaced by</div>`;
    if (aid != null) {
      h += `<div class="dd-field" style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;">`
        + `<span>${esc(_actionLabel(aid))}</span>`
        + `<button onclick="event.stopPropagation();switchView('flow');selectAction(${aid});`
        + `document.querySelector('.action-card[data-action-id=&quot;${aid}&quot;]')?.scrollIntoView({behavior:'smooth',block:'center'});"`
        + ` style="background:var(--bg-deep);border:1px solid var(--accent);color:var(--accent);font-size:10px;`
    ...
    const anchors = [];
    if (residual.target?.target_type === 'artifact' && residual.target.target_id) anchors.push(residual.target.target_id);
    for (const id of (residual.related_artifact_ids || [])) if (id && !anchors.includes(id)) anchors.push(id);
    if (anchors.length) {
      h += `<div class="dd-label">Where it bites</div><div class="dd-field" style="display:flex;gap:6px;flex-wrap:wrap;">`
        + anchors.map((id) => `<button onclick="event.stopPropagation();selectDAGNode('${esc(id)}');"`
            + ` title="Show ${esc(_artifactLabel(id))} in the graph"`
            + ` style="background:var(--bg-deep);border:1px solid var(--border);color:var(--text-secondary);`
            + `font-size:10px;padding:2px 8px;border-radius:4px;cursor:pointer;">${esc(_artifactLabel(id))}</button>`).join('')
        + `</div>`;
    }
    detail.innerHTML = h;
    return;
