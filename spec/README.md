...
| [`ORACLE_SPEC_v0_2.md`](ORACLE_SPEC_v0_2.md) | 0.2 | Draft | **Who produced the evidence.** The oracle as a **generic type**, fixed by contract not mechanism — any invocable evidence producer: a formal reasoner, a test runner, a static analyzer, **a database or reference-data store** (`monitor`), an LLM-judge, a human attestor — with two orthogonal classifiers that travel with every result: `oracle_type` (the mechanism; an *open* set of standard names) and `evidence_strength` (the guarantee: proof > sat > tests > static_analysis > attested). v0.2 adds per-result **attribution**, the generic **evidence fingerprint** (freshness for any oracle's evidence), an optional `probe`, the `Observation` artifact, and strength / type / producer policy predicates. A *reasoner* is the formal, proof-producing subtype. |
| [`SDK_SPEC_v0_1.md`](SDK_SPEC_v0_1.md) | 0.1 | Draft | **Emit a trace as you go.** The runtime contract for agents that speak ponens natively — a `Session` that records actions, artifacts, and lineage while the work happens and invokes oracles for evidence, instead of reconstructing a trace from a transcript afterwards. |
